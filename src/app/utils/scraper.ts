...
async function getCachedContent(url: string): Promise<ScrapedContent | null> {
  try {
    ...
    // Handle both string and oject responses from Redis
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let parsed: any;
...
async function cacheContent(
  ...
  try {
    ...
    const serialzed = JSON.stringify(content);

    if (serialzed.length > MAX_CACHE_SIZE) {
      logger.warn(
        `Content too large to cache for URL: ${url} (${serialzed.length} bytes)`
      );
      return;
    }

    await redis.set(cacheKey, serialzed, { ex: CACHE_TTL });
    logger.info(
      `Successfully cached content for: ${url} (${serialzed.length} bytes, TTL: ${CACHE_TTL})`
    );
  } catch (error) {
